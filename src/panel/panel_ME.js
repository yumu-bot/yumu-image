...
export async function panel_ME(data = {
    ...
    // 导入文字
    svg = setText(svg, getPanelNameSVG('maimai Best Performance (!ymmb)', 'MB'), reg_index);
    ...
    svg = setImage(svg, 0, 0, 1920, 320, banner, reg_banner_blurred, 0.7);
...
const component_E3 = (
    ...
    const delta = (data?.fit > 0) ? (' (' + getSignNumber(d, (d) => { return floor(Math.abs(d), 2) }) +  + ')') : ''
