...
export async function panel_A6(data = {
    ...
    const markdown = RenameExportFilePath(originalMarkdown);
...
function RenameExportFilePath(md = "") {
    const defaultPath = "D:\\ExportFileV3\\";
    const environmentPath = process.env.EXPORT_FILE;

    const hasExportFile = environmentPath != null && environmentPath !== "";

    return hasExportFile ? md.replace(defaultPath, environmentPath) : md;
}
