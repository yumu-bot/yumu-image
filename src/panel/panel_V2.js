    ...
    })
{
    ...
    chunks.forEach((chunk, i) => {
        const chunk_start_beat = start_bar * 4 + i * beats_per_lane;

        // 寻找在当前 chunk 开始之前的最后一条线
        let last_point_before = null;
        for (let j = full_line.length - 1; j >= 0; j--) {
            // 使用 epsilon 避免浮点误差
            if (full_line[j].beat <= chunk_start_beat + 1e-4 && full_line[j].standard_sv != null) {
...
<g>
    ...
    ${getPanelNameSVG('Beatmap View (!ymv) ~ Not even close to donscore', sv_mode ? 'VV' : 'V', request_time)}
