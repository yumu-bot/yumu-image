...
export async function panel_B2(data = {
    ...
    svg = implantSvgBody(svg, 0, 0, s_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, s_arr).toFixed(3), 75 + 1370, -50 + 465, 24, 'center baseline', '#fff'), reg_center);
    const j_arr = data.mapMinus.jack || [];
    const j_c = PanelDraw.LineChart(j_arr, 0, 0, 1370 + 170, 465, 150, 95, '#0071BC', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, j_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, j_arr).toFixed(3), 75 + 1370 + 170, -50 + 465, 24, 'center baseline', '#fff'), reg_center);
    const b_arr = data.mapMinus.bracket || [];
    const b_c = PanelDraw.LineChart(b_arr, 0, 0, 1370 + 340, 465, 150, 95, '#0054A6', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, b_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, b_arr).toFixed(3), 75 + 1370 + 340, -50 + 465, 24, 'center baseline', '#fff'), reg_center);

    const h_arr = data.mapMinus.handLock || [];
    const h_c = PanelDraw.LineChart(h_arr, 0, 0, 1370, 465 + 115, 150, 95, '#8DC73D', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, h_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, h_arr).toFixed(3), 75 + 1370, -50 + 465 + 115, 24, 'center baseline', '#fff'), reg_center);
    const o_arr = data.mapMinus.overlap || [];
    const o_c = PanelDraw.LineChart(o_arr, 0, 0, 1370 + 170, 465 + 115, 150, 95, '#39B449', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, o_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, o_arr).toFixed(3), 75 + 1370 + 170, -50 + 465 + 115, 24, 'center baseline', '#fff'), reg_center);
    const r_arr = data.mapMinus.release || [];
    const r_c = PanelDraw.LineChart(r_arr, 0, 0, 1370 + 340, 465 + 115, 150, 95, '#00A550', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, r_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, r_arr).toFixed(3), 75 + 1370 + 340, -50 + 465 + 115, 24, 'center baseline', '#fff'), reg_center);

    const e_arr = data.mapMinus.shield || [];
    const e_c = PanelDraw.LineChart(e_arr, 0, 0, 1370, 465 + 345, 150, 95, '#F8941B', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, e_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, e_arr).toFixed(3), 75 + 1370, -50 + 465 + 345, 24, 'center baseline', '#fff'), reg_center);
    const c_arr = data.mapMinus.riceDensity || [];
    const c_c = PanelDraw.LineChart(c_arr, 0, 0, 1370 + 170, 465 + 345, 150, 95, '#F26420', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, c_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, c_arr).toFixed(3), 75 + 1370 + 170, -50 + 465 + 345, 24, 'center baseline', '#fff'), reg_center);
    const d_arr = data.mapMinus.longNoteDensity || [];
    const d_c = PanelDraw.LineChart(d_arr, 0, 0, 1370 + 340, 465 + 345, 150, 95, '#ED1622', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, d_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, d_arr).toFixed(3), 75 + 1370 + 340, -50 + 465 + 345, 24, 'center baseline', '#fff'), reg_center);

    const k_arr = data.mapMinus.speedJack || [];
    const k_c = PanelDraw.LineChart(k_arr, 0, 0, 1370, 465 + 460, 150, 95, '#EC008C', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, k_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, k_arr).toFixed(3), 75 + 1370, -50 + 465 + 460, 24, 'center baseline', '#fff'), reg_center);
    const i_arr = data.mapMinus.trill || [];
    const i_c = PanelDraw.LineChart(i_arr, 0, 0, 1370 + 170, 465 + 460, 150, 95, '#92248F', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, i_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.max.apply(Math, i_arr).toFixed(3), 75 + 1370 + 170, -50 + 465 + 460, 24, 'center baseline', '#fff'), reg_center);
    const u_arr = data.mapMinus.burst || [];
    const u_c = PanelDraw.LineChart(u_arr, 0, 0, 1370 + 340, 465 + 460, 150, 95, '#662B91', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, u_c, reg_center);
    svg = replaceText(svg, torus.getTextPath(Math.round(Math.max.apply(Math, u_arr)).toFixed(3), 75 + 1370 + 340, -50 + 465 + 460, 24, 'center baseline', '#fff'), reg_center);
