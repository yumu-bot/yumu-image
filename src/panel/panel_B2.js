...
export async function panel_B2(data = {
    ...
    const s_c = PanelDraw.LineChart(s_arr, 0, 0, 1370, 350, 150, 95, '#00A8EC', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, s_c, reg_right);
    const j_arr = data.mapMinus.jack || [];
    const j_c = PanelDraw.LineChart(j_arr, 0, 0, 1370 + 170, 350, 150, 95, '#0071BC', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, j_c, reg_right);
    const b_arr = data.mapMinus.bracket || [];
    const b_c = PanelDraw.LineChart(b_arr, 0, 0, 1370 + 340, 350, 150, 95, '#0054A6', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, b_c, reg_right);

    const h_arr = data.mapMinus.handLock || [];
    const h_c = PanelDraw.LineChart(h_arr, 0, 0, 1370, 350 + 115, 150, 95, '#8DC73D', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, h_c, reg_right);
    const o_arr = data.mapMinus.overlap || [];
    const o_c = PanelDraw.LineChart(o_arr, 0, 0, 1370 + 170, 350 + 115, 150, 95, '#39B449', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, o_c, reg_right);
    const r_arr = data.mapMinus.release || [];
    const r_c = PanelDraw.LineChart(r_arr, 0, 0, 1370 + 340, 350 + 115, 150, 95, '#00A550', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, r_c, reg_right);

    const e_arr = data.mapMinus.shield || [];
    const e_c = PanelDraw.LineChart(e_arr, 0, 0, 1370, 350 + 345, 150, 95, '#F8941B', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, e_c, reg_right);
    const c_arr = data.mapMinus.riceDensity || [];
    const c_c = PanelDraw.LineChart(c_arr, 0, 0, 1370 + 170, 350 + 345, 150, 95, '#F26420', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, c_c, reg_right);
    const d_arr = data.mapMinus.longNoteDensity || [];
    const d_c = PanelDraw.LineChart(d_arr, 0, 0, 1370 + 340, 350 + 345, 150, 95, '#ED1622', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, d_c, reg_right);

    const k_arr = data.mapMinus.speedJack || [];
    const k_c = PanelDraw.LineChart(k_arr, 0, 0, 1370, 350 + 460, 150, 95, '#EC008C', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, k_c, reg_right);
    const i_arr = data.mapMinus.trill || [];
    const i_c = PanelDraw.LineChart(i_arr, 0, 0, 1370 + 170, 350 + 460, 150, 95, '#92248F', 1, 0.3, 3)
    svg = implantSvgBody(svg, 0, 0, i_c, reg_right);
    const u_arr = data.mapMinus.burst || [];
    const u_c = PanelDraw.LineChart(u_arr, 0, 0, 1370 + 340, 350 + 460, 150, 95, '#662B91', 1, 0.3, 3)
