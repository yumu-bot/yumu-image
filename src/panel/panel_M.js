...
export async function panel_M(data = {
    ...
    // 下面俩我没看懂, 查询条件都一样,我直接给你上面查询结果中符合的的第一条?
    // 另外麻烦写注释有中英混杂的情况下, 尽量在英文单词前后加个空格, 挨着的是标点符号不用加, 比如 this, 想知道为什么的话你双击一下 this 与这个this
    most_recent_ranked_beatmap: {},//需要判断，如果玩家的ranked_and_approved_beatmapset_count不为0，则走search，搜索https://osu.ppy.sh/beatmapsets?q=creator%3D（uid）&s=any，筛选出 ranked/qualified中。上传时间最靠前，但是谱面主（host）是uid所指的人的那张谱面即可。如果没有结果返回空
