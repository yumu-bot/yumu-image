...
export async function panel_A5(data = {
    ...
    // 面板文字
    const panel_name = getPanelNameSVG('Multi Scores (!ymp / !ymr)', 'Scores', 'v0.3.2 FT');
    ...
    for (const i in data.score) {
        ...
        const calcPP = await calcPerformancePoints(v.beatmap.id, score_statistics, v.mode, !(v.beatmap.ranked && v.beatmap.ranked === 1));
