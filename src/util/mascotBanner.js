...
import {clamp, getImageFromV3} from "./util.js";
...
const typeCBannerTotal = 8;
// const mascotBGTotal = 13; //吉祥物 BG 数量

export function getBannerLocal(specified = null, max = typeCBannerTotal, type = "c", format = "webp") {
    let num

    if (Number.isInteger(specified)) {
        num = clamp(specified, max, 1);
    } else {
        num = getRandom(max)
    }

    const file = `${type}${num}.${format}`;

    return getImageFromV3(path.join("Banner", file))
}
